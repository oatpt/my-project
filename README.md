...
ชุดจอแสดงผล 6 ตัว (`ProTech_-_จอแสดงผล_แก้ไขตามTOR.docx`) ไม่มีใบเสนอราคา `.xlsx`
ราคาเขียนอยู่ในเอกสาร word ของตัวเอง จึงเก็บไว้ที่ `"price"` ของสินค้านั้นใน
`products.json` แล้วต่อท้ายตารางให้เอง — LumaGrid, PixelCore, ScoreEngine, FiberLink,
PowerGuard, FrameWorks

```jsonc
"price": 100240,       // ราคาต่อหน่วย (บาท)
"priceUnit": "เมตร"    // ใส่เฉพาะตอนที่ไม่ได้คิดเป็นชุด — จะขึ้นเป็น "94 / เมตร"
```

กติกาเดิมไม่เปลี่ยน คือทุกแถวต้องเป็นสินค้าที่มีหน้าของตัวเอง สินค้าตัวไหนมีทั้ง
`"quotationItem"` และ `"price"` จะขึ้นสองแถว ให้เลือกใช้อย่างใดอย่างหนึ่ง

#### สินค้าที่มีหน้าเว็บแต่ไม่ขึ้นหน้าราคา

ชุดเสียงตามสาย 6 ตัว (`ProTech - เสียงตามสาย Rev 3.docx` — ชุดกระจายสัญญาณใยแก้วนำแสง
ส่วนกลาง, ชุดเชื่อมต่อโครงข่ายฯ ภาคสนาม, ชุดรับและขยายสัญญาณเสียงผ่านเครือข่าย,
ชุดลำโพงฮอร์น, ตู้ติดตั้งอุปกรณ์ภาคสนาม, สายใยแก้วนำแสง) **ตั้งใจไม่ใส่ `"price"`**
จึงมีหน้ารายละเอียดของตัวเองแต่ไม่ขึ้นในตารางราคา ถ้าวันหลังอยากให้ขึ้น ใส่ `"price"`
กลับเข้าไปได้เลย ราคาอยู่ในเอกสาร word

ชุดนี้ในเอกสารไม่มีชื่ออังกฤษกับหัวข้อ "จุดเด่น" จึงใช้ชื่อไทยเป็นชื่อหลัก และมีแต่
หัวข้อ "Specification" ตามที่เอกสารเขียนไว้ (Rev 3 ตัดจำนวนชุดออกจากทุกรายการ เว็บจึง
ไม่มีจำนวนด้วย) รูปของสินค้าทุกตัวดึงมาจากรูปในเอกสารเอง
