...
| `public/files/_quotations/` | `.pdf` ที่ export จาก Excel | **ไม่ได้ลิงก์จากหน้าเว็บแล้ว** — ลูกค้าใช้ปุ่ม "ขอใบเสนอราคา" แทน เก็บไว้เป็นไฟล์อ้างอิง |
| `src/data/quotations.json` | ข้อมูลในไฟล์ `.xlsx` แปลงเป็น JSON | ตั้งต้นของปุ่ม "ขอใบเสนอราคา" (สร้างอัตโนมัติ ไม่ต้องแก้เอง) |
...
### ปุ่ม "ขอใบเสนอราคา" ในหน้าสินค้า

ท้ายหน้ารายละเอียดสินค้ามีปุ่มเปิดใบเสนอราคาของโครงการนั้นขึ้นมาเป็นฟอร์ม แก้ตัวเลข
แล้วสั่งพิมพ์หรือบันทึกเป็น PDF ได้จากหน้าเว็บเลย กติกาของฟอร์ม:

- **จำนวนตั้งไว้ที่ 1 ทุกรายการ** ตารางที่เห็นตอนเปิดจึงเป็นราคาต่อหน่วยล้วน ๆ
  ราคารวมแต่ละแถวและยอดรวมคิดให้ใหม่ทันที
- **วันที่เป็นวันที่วันนี้เสมอ** หัวเอกสารกับหมายเหตุแสดงเฉย ๆ แก้ไม่ได้
- **รายการตามราคากลาง ICT** (แถวที่ระบายสีเขียวในไฟล์ excel) แก้ได้เฉพาะจำนวน
  ชื่อ หน่วย ราคา ล็อกไว้ และลบออกไม่ได้
- **รายการนอกราคากลาง** แก้ได้ทุกช่อง เพิ่ม/ลบได้ กด "เริ่มใหม่" กลับไปค่าตั้งต้น
- กระดาษที่พิมพ์ออกมาเป็น **Letter แนวนอน** ขอบเท่าไฟล์ต้นฉบับ หน้าตาเหมือน PDF
  ที่ export จาก Excel

สินค้าตัวไหนใช้ใบเสนอราคาใบไหน ดูที่ค่า `"quotation"` ของสินค้านั้นใน `products.json`
(ค่าคือชื่อไฟล์ `.xlsx` ที่ตัดนามสกุลออก) ไม่ใส่ = ไม่มีปุ่มนี้

**รูปแบบเอกสารมาจากไฟล์ excel ทั้งหมด** ทั้งฟอนต์ ขนาด สีพื้น สีตัวอักษร ความกว้าง
คอลัมน์ และรูปแบบตัวเลข ถูกอ่านออกมาจาก `.xlsx` (ดู `scripts/lib/quotation-xlsx.mjs`)
แล้วใส่ให้แต่ละช่องตอนแสดงผล แก้หน้าตาที่ไฟล์ excel แล้วสั่ง `npm run dev` ใหม่
ทั้งหน้าเว็บและ PDF จะเปลี่ยนตาม ไม่ต้องแก้ CSS

ข้อควรรู้: ไฟล์ต้นฉบับใช้ฟอนต์ **TH Sarabun New** เครื่องที่ไม่มีฟอนต์นี้จะแสดง
(และพิมพ์) ด้วยฟอนต์สำรอง หน้าตาจะเพี้ยนจากไฟล์ excel ไปเล็กน้อย
